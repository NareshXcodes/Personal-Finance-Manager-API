...
def get_budget_by_id(id: int, db: SessionDep):
    budget = db.query(Budget).filter(Budget.id == id).first()
    if not budget:
        return Response(status_code=status.HTTP_404_NOT_FOUND, content="Budget not found")
    return budget


@router.post("/", status_code = status.HTTP_201_CREATED )
def create_budget(new_budget : BudgetCreate , db:SessionDep):
    #new_budget = Budget(name = new_budget.name , category = new_budget.category , monthly_limit = new_budget.monthly_limit)
    new_budget =  Budget(**new_budget.model_dump())
    db.add(new_budget)
    db.commit()
...
def delete_budget(id:int,db:SessionDep):
    deleted_budget = db.query(Budget).filter(Budget.id == id)

    if deleted_budget.first() == None:
        raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, detail="Budget not found")

    deleted_budget.delete(synchronize_session=False)
    db.commit()
    return Response(status_code = status.HTTP_204_NO_CONTENT)


@router.put("/{id}")
def update_budget(id:int,update_budget : BudgetUpdate,db:SessionDep):
